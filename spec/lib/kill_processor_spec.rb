...
describe KillProcessor do
  ...
  describe '#add_kill' do
    context 'when player Kill another player' do
      before do
        client_2_begun
        client_3_begun
        game_parser.process(LogLine.new(kill2_line))
      end

      it 'increase the total kills by one' do
        expect(game_parser.total_kills).to eq 1
      end

      it 'increase the kills of the player 2 by one' do
        expect(game_parser.players.first.kills).to eq 1
      end
    end
  end
  ...
  context 'when the <world> kill player 2' do
    before do
      client_2_begun
      game_parser.process(LogLine.new(kill2_world_line))
    end
    ...
    it 'not appear in the player list' do
      game_parser.players.each { |player| expect(player.name).not_to eq KillLogLine::WORLD }
    end
  end
  ...
  context 'when player kill itself' do
    before do
      client_3_begun
      game_parser.process(LogLine.new(self3_kill_line))
    end
